...
quad.params.boot.df <- data.frame(cf.q = numeric(n.iterations), cf.T0 = numeric(n.iterations), cf.Tm = numeric(n.iterations))
for( i in 1:nrow(quad.params)){
  quad.params.boot.df[,i] <- rnorm(7500, quad.params$mean[i], quad.params$sigma[i])
}
head(quad.params.boot.df)
...
quad.boot.outputs <- matrix(nrow = 7500, ncol = 501)
...
for(l in 1:nrow(quad.boot.outputs)){
  quad.boot.outputs[l,] <- QUAD(cf.q = quad.params.boot.df[l,1], cf.T0 = quad.params.boot.df[l,2], cf.Tm = quad.params.boot.df[l,3], Treatment = temp.gradient)
...
plot.new()
plot(temp.gradient, quad.boot.outputs[3,], ylim = c(0, 60), type = 'n')
