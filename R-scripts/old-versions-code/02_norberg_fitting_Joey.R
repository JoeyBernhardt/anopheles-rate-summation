...
traits <- read_csv("data-raw/fluc.program.trait.means.csv")


t_constant <- traits %>% 
	filter(DTR == "constant") %>% 
	mutate(temperature = TempK - 273.15)

dat.full <- t_constant %>% 
	mutate(curve.id = "1") %>% 
	rename(growth.rate = bite.rate) %>% 
...
predictions %>% 
	...
	ggplot(aes(x = temperature, y = predicted_growth)) + geom_point() +
	geom_line()
