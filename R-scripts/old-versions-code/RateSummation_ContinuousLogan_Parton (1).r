...
mainDir = "C:/Users/Kerri/Desktop/Fluctuation_RateSummationProject"
setwd(mainDir)
...
##A sinusoial  relationship during the day followed by an exponential decay throughout the course of the night
...
#Plot of all the temperature programs (use the same code loop as when I plot all of the estimates)
#Just as an example I currently plot mean temp of 24C with a dtr of 9C
ggplot(temp.programs.2, aes(x=hr, y=X24.dtr9))+
  geom_line()+
  geom_hline(aes(yintercept= mean(X24.dtr9)), lty = "dashed")+
  geom_hline(aes(yintercept= 24), lty = "dotted", colour = "red") #the mean of the col values should correspond to the stated mean
...
#To avoid issues with data conversion; I will export these as csv files and then reload them in R.
