...
boxplot(EFD.1stGC ~ temp, data = data.all) # briere
...
boxplot(EFD.1stGC ~ temp, data = data.block.temp) # briere


############## Trait name and parameter symbol - model type (e.g. quadratic, Briere)

############Estimated EFD (1st GC) - Briere - no Truncation [all data]
# Get data
data.specific <- with(data.all, data.frame('T' = temp, 'trait' = EFD.1stGC)) # subset specific trait data from complete list
...
# Save model output 
save(model.out, file = "saved posteriors/est.EFD_briere_noT_all_uniform.Rdata")

# Plot trait data, model mean and CIs
plot(trait ~ jitter(T, 0.5), xlim = c(0, 45), data = data.specific, 
     ylab = "est. EFD", xlab = expression(paste("Temperature (",degree,"C)")))
...
#################################### Estimated EFD - Briere - with Truncation [all data]
# Run JAGS - **select correct model file**
...
# Save model output 
save(model.out, file = "saved posteriors/est.EFD_briere_withT_all_uniform.Rdata")
...
############ Estimated EFD - Briere - no Truncation [data means]
# Get data
...
# Save model output 
save(model.out, file = "saved posteriors/est.EFD_briere_noT_means_uniform.Rdata")
...
#################################### Estimated daily eggs - Briere - with Truncation [data means]
# Run JAGS - **select correct model file**
model.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, model.file="briere_T.txt",
                  n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni, DIC=T, working.directory=getwd())
...
# Save model output 
save(model.out, file = "saved posteriors/est.EFD_briere_withT_means_uniform.Rdata")
...
####################Estimated daily eggs -Quadratic - with Truncation [data means]
# Run JAGS - **select correct model file**
...
# Examine model output & run diagnostics
model.out$BUGSoutput$summary[1:10,]
est.EFD.DIC <- model.out$BUGSoutput$DIC
mcmcplot(model.out)

# Save model output 
save(model.out, file = "saved posteriors/est.EFD_quad_withT_means_uniform.Rdata")
