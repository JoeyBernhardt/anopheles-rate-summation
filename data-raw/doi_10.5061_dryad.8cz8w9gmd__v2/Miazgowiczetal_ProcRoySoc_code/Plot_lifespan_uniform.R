...
load("saved posteriors/lifespan_quad_noT_all_uniform.Rdata")
lifespan.all.noT.preds <- model.out$BUGSoutput$sims.list$z.trait.mu.pred
load("saved posteriors/lifespan_quad_withT_all_uniform.Rdata")
lifespan.all.withT.preds <- model.out$BUGSoutput$sims.list$z.trait.mu.pred
load("saved posteriors/lifespan_quad_noT_means_uniform.Rdata")
lifespan.mean.noT.preds <- model.out$BUGSoutput$sims.list$z.trait.mu.pred
"saved posteriors/lifespan_quad_withT_means_uniform.Rdata"
lifespan.mean.withT.preds <- model.out$BUGSoutput$sims.list$z.trait.mu.pred
...
pdf("plots/lifespan_fits_raw.pdf", width = 10, height = 12)
par(mfrow=c(4,3))
trait.plots("lifespan", "lifespan_quad_noT_all_uniform.Rdata", data.all, "Lifespan")
trait.plots("lifespan", "lifespan_quad_withT_all_uniform.Rdata", data.all, "Lifespan")
trait.plots("lifespan", "lifespan_quad_noT_means_uniform.Rdata", data.block.temp, "Lifespan")
trait.plots("lifespan", "lifespan_quad_withT_means_uniform.Rdata", data.block.temp, "Lifespan")
par(mfrow = c(1,1))
dev.off()
##########
...
###########
pdf("plots/lifespan_fits_mean_se.pdf", width = 10, height = 12)
par(mfrow = c(4,3))
trait.plots.meanse("lifespan", "lifespan_quad_noT_all_uniform.Rdata", data.all, "Lifespan")
trait.plots.meanse("lifespan", "lifespan_quad_withT_all_uniform.Rdata", data.all, "Lifespan", cols = "purple", add = "true")
legend('topleft', legend = c("all_noT", "all_withT"), lty = 1, col = c("black", "purple"), bty = 'n')
mtext("A", side = 3, at = -5, cex = 1.8, line = 2)

trait.plots.meanse("lifespan", "lifespan_quad_noT_means_uniform.Rdata", data.block.temp, "Lifespan")
trait.plots.meanse("lifespan", "lifespan_quad_withT_means_uniform.Rdata", data.block.temp, "Lifespan", cols = "purple", add = "true")
legend('topleft', legend = c("means_noT", "means_withT"), lty = 1, col = c("black", "purple"), bty = 'n')
mtext("B", side = 3, at = -5, cex = 1.8, line = 2)
plot(NULL)

trait.plots.meanse("lifespan", "lifespan_quad_noT_all_uniform.Rdata", data.all, "Lifespan")
trait.plots.meanse("lifespan", "lifespan_quad_noT_means_uniform.Rdata", data.block.temp, "Lifespan", cols = "royalblue", add = "true")
legend('topleft', legend = c("all_noT", "means_noT"), lty = 1, col = c("black", "royalblue"), bty = 'n')
mtext("C", side = 3, at = -5, cex = 1.8, line = 2)

trait.plots.meanse("lifespan", "lifespan_quad_withT_all_uniform.Rdata", data.all, "Lifespan")
trait.plots.meanse("lifespan", "lifespan_quad_withT_means_uniform.Rdata", data.block.temp, "Lifespan", cols = "royalblue", add = "true")
legend('topleft', legend = c("all_withT", "means_withT"), lty = 1, col = c("black", "royalblue"), bty = 'n')
mtext("D", side = 3, at = -5, cex = 1.8, line = 2)
plot(NULL)

trait.plots.meanse("lifespan", "lifespan_quad_noT_all_uniform.Rdata", data.all, "Lifespan")
trait.plots.meanse("lifespan", "lifespan_quad_withT_means_uniform.Rdata", data.block.temp, "Lifespan", cols = "darkcyan", add = "true")
legend('topleft', legend = c("all_noT", "means_withT"), lty = 1, col = c("black","darkcyan"), bty = 'n')
mtext("E", side = 3, at = -5, cex = 1.8, line = 2)

trait.plots.meanse("lifespan", "lifespan_quad_withT_means_uniform.Rdata", data.block.temp, "Lifespan")
trait.plots.meanse("lifespan", "lifespan_quad_noT_means_uniform.Rdata", data.block.temp, "Lifespan", cols = "firebrick", add = "true")
trait.plots.meanse("lifespan", "lifespan_quad_withT_all_uniform.Rdata", data.all, "Lifespan", cols = "dodgerblue", add = "true")
trait.plots.meanse("lifespan", "lifespan_quad_noT_all_uniform.Rdata", data.all, "Lifespan", cols = "darkgreen", add = "true")
