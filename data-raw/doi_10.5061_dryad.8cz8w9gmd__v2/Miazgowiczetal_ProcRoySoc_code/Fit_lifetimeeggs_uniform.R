...
boxplot(lifetime.eggs ~ temp, data = data.all) # quad
...
boxplot(lifetime.eggs ~ temp, data = data.block.temp) # briere?


############## Trait name and parameter symbol - model type (e.g. quadratic, Briere)

############lifetime eggs- quadratic - no Truncation [all data]
# Get data
data.specific <- with(data.all, data.frame('T' = temp, 'trait' = lifetime.eggs)) # subset specific trait data from complete list
...
DIC.lifeeggs.quadnoT <- model.out$BUGSoutput$DIC
mcmcplot(model.out)

# Save model output 
save(model.out, file = "saved posteriors/lifeeggs_quad_noT_all_uniform.Rdata")

# Plot trait data, model mean and CIs
plot(trait ~ jitter(T, 0.5), xlim = c(0, 45), data = data.specific, 
     ylab = "life eggs", xlab = expression(paste("Temperature (",degree,"C)")))
...
# Examine model output & run diagnostics
model.out$BUGSoutput$summary[1:10,]
DIC.lifeeggs.quadwithT <- model.out$BUGSoutput$DIC
mcmcplot(model.out)

# Save model output 
save(model.out, file = "saved posteriors/lifeeggs_quad_withT_all_uniform.Rdata")
...
# Examine model output & run diagnostics
model.out$BUGSoutput$summary[1:10,]
DIC.lifeeggs.briere.noT.all <- model.out$BUGSoutput$DIC
mcmcplot(model.out)

# Save model output 
save(model.out, file = "saved posteriors/lifeeggs_briere_noT_all_uniform.Rdata")
...
#################################### Lifespan - briere - with Truncation [all data]
# Run JAGS - **select correct model file**
...
# Examine model output & run diagnostics
model.out$BUGSoutput$summary[1:10,]
DIC.lifeeggs.all.brierewithT <- model.out$BUGSoutput$DIC
mcmcplot(model.out)

# Save model output 
save(model.out, file = "saved posteriors/lifeeggs_briere_withT_all_uniform.Rdata")
...
# Examine model output & run diagnostics
model.out$BUGSoutput$summary[1:10,]
DIC.lifeeggs.means.quadnoT <- model.out$BUGSoutput$DIC
mcmcplot(model.out)

# Save model output 
save(model.out, file = "saved posteriors/lifeeggs_quad_noT_means_uniform.Rdata")
...
#################################### Lifetime eggs - quad- with Truncation [data means]
# Run JAGS - **select correct model file**
model.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, model.file="quad_T.txt",
                  n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni, DIC=T, working.directory=getwd())
...
# Examine model output & run diagnostics
model.out$BUGSoutput$summary[1:10,]
DIC.lifeeggs.means.quadwithT <- model.out$BUGSoutput$DIC
mcmcplot(model.out)

# Save model output 
save(model.out, file = "saved posteriors/lifeeggs_quad_withT_means_uniform.Rdata")
...
############ Lifespan - briere - no Truncation [data means]
# Get data
data.specific <- with(data.block.temp, data.frame('T' = temp, 'trait' = lifetime.eggs)) # subset specific trait data from complete list
data <- data.specific # assign trait data to variable 'data'
...
# Examine model output & run diagnostics
model.out$BUGSoutput$summary[1:10,]
DIC.lifeeggs.means.brierenoT <- model.out$BUGSoutput$DIC
mcmcplot(model.out)

# Save model output 
save(model.out, file = "saved posteriors/lifeeggs_briere_noT_means_uniform.Rdata")
...
# Examine model output & run diagnostics
model.out$BUGSoutput$summary[1:10,]
DIC.lifeeggs.means.brierewithT <- model.out$BUGSoutput$DIC
mcmcplot(model.out)

# Save model output 
save(model.out, file = "saved posteriors/lifeeggs_briere_withT_means_uniform.Rdata")
