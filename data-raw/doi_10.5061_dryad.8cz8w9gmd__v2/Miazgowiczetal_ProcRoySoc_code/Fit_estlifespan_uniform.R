...
boxplot(lifespan ~ temp, data = data.mu) # quad
...
############Estimated lifespan - quadratic - no Truncation [all data]
# Get data
data.specific <- with(data.mu, data.frame('T' = temp, 'trait' = lifespan)) # subset specific trait data from complete list
...
# Save model output 
save(model.out, file = "saved posteriors/est.lifespan_quad_noT_all_uniform.Rdata")

# Plot trait data, model mean and CIs
plot(trait ~ jitter(T, 0.5), xlim = c(0, 45), data = data.specific, 
     ylab = "est. lifespan", xlab = expression(paste("Temperature (",degree,"C)")))
...
#################################### estimated Lifespan - quad - with Truncation [all data]
# Run JAGS - **select correct model file**
...
# Save model output 
save(model.out, file = "saved posteriors/est.lifespan_quad_withT_all_uniform.Rdata")
...
#############################estimated lifespan - briere- with truncation [all data]
# Run JAGS - **select correct model file**
...
# Examine model output & run diagnostics
model.out$BUGSoutput$summary[1:10,]
est.lifespan.DIC <- model.out$BUGSoutput$DIC
mcmcplot(model.out)

# Save model output 
save(model.out, file = "saved posteriors/est.lifespan_briere_withT_all_uniform.Rdata")
