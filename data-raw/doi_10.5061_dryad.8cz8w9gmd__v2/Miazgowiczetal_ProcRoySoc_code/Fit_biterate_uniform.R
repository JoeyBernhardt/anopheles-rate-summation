...
## Updated by KM on April 14,2020 to determine dic value associated with Quad fit
##
...
boxplot(bite.rate ~ temp, data = data.all) # briere?
boxplot(est.bite ~ temp, data = data.all) # briere
...
boxplot(bite.rate ~ temp, data = data.block.temp) # briere?

############## Trait name and parameter symbol - model type (e.g. quadratic, Briere)

############ Lifetime Bite rate - Briere - no Truncation [all data]
# Get data
data.specific <- with(data.all, data.frame('T' = temp, 'trait' = bite.rate)) # subset specific trait data from complete list
...
# Save model output 
save(model.out, file = "saved posteriors/bite.rate_briere_noT_all_uniform.Rdata")
...
#################################### Lifetime Bite rate - Briere - with Truncation [all data]
# Run JAGS - **select correct model file**
...
# Save model output 
save(model.out, file = "saved posteriors/bite.rate_briere_withT_all_uniform.Rdata")
...
############ Lifetime Bite rate - Briere - no Truncation [data means]
# Get data
...
# Save model output 
save(model.out, file = "saved posteriors/bite.rate_briere_noT_means_uniform.Rdata")
...
#################################### Lifetime Bite rate - Briere - with Truncation [data means]
# Run JAGS - **select correct model file**
model.out <- jags(data=jag.data, inits=inits, parameters.to.save=parameters, model.file="briere_T.txt",
                  n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni, DIC=T, working.directory=getwd())
...
# Save model output 
save(model.out, file = "saved posteriors/bite.rate_briere_withT_means_uniform.Rdata")
...
######################fit a quadratic to compare DIC values
#######Lifetime biting rate , quadratic model, with truncation
# Run JAGS - **select correct model file**
...
# Examine model output & run diagnostics
model.out$BUGSoutput$summary[1:10,]
bite.rate.quad.DIC <- model.out$BUGSoutput$DIC
mcmcplot(model.out)

# Save model output 
save(model.out, file = "saved posteriors/bite.rate_quad_withT_means_uniform.Rdata")
